...
class PlayViewController4: UIViewController {
    public var playView = PlayView4()
...
//        rocket.position = CGPoint(x: scene.size.width / 2, y: scene.size.height-500)
...
//MARK: обратный отчет
private extension PlayViewController4 {
        ...
        if remainingTime > 1 {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                ...
                UIView.animate(withDuration: 3.0, delay: 0, options: .curveEaseInOut, animations: {
                    ...
                    self.playView.rocketImageView.image = UIImage(named: "dfg")
                }, completion: nil)
            }
        }
...
private extension PlayViewController4 {
    ...
    @objc func animateRocket() {
        ...
        rocketImageView.frame.origin.x = view.bounds.midX - rocketImageView.frame.width / 2
        rocketImageView.frame.origin.y = view.bounds.height-215
    }
}
